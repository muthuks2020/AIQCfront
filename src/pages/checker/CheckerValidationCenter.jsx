...
styleTag.textContent = `
  ...
  @keyframes ckProgressShrink { from { width: 100%; } to { width: 0%; } }
...
export default function CheckerValidationCenter() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setInspections(prev => prev.map(i =>
        i.id === selectedInspection.id ? { ...i, status: "approved", checkerRemarks: actionRemarks, checkerDate: updatedDate, checkerName: user.name } : i
      ));
      setShowActionModal(null);
      setActionRemarks("");

      setActionSuccess({ type: "approved", irNumber: selectedInspection.irNumber, partName: selectedInspection.partName });
      setTimeout(() => {
        setActionSuccess(null);
        setSelectedInspection(null);
        setCurrentView("dashboard");
        setActiveTab("approved");
        showToast(`${selectedInspection.irNumber} approved. Store transfer can now proceed.`, "success");
      }, 1800);
    } catch (e) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      const updatedDate = new Date().toISOString();
      setInspections(prev => prev.map(i =>
        i.id === selectedInspection.id ? { ...i, status: "rejected", checkerRemarks: actionRemarks, rejectionCategory, rejectionReason, checkerDate: updatedDate, checkerName: user.name, rejectionCount: (i.rejectionCount || 0) + 1, returnedToMaker: true } : i
      ));
      setShowActionModal(null);
      setActionRemarks("");
      ...
      setActionSuccess({ type: "rejected", irNumber: selectedInspection.irNumber, partName: selectedInspection.partName, makerName: selectedInspection.makerName });
      setTimeout(() => {
        setActionSuccess(null);
        setSelectedInspection(null);
        setCurrentView("dashboard");
        setActiveTab("rejected");
        showToast(`${selectedInspection.irNumber} rejected and returned to ${selectedInspection.makerName}'s queue.`, "warning");
      }, 1800);
    } catch (e) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: colors.background.secondary, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      {actionSuccess && <SuccessOverlay data={actionSuccess} />}
...
const SuccessOverlay = ({ data }) => {
  ...
  return (
    <div style={{
      ...
      <div className="ck-scale-in" style={{
        background: "white", borderRadius: borderRadius.xl, padding: "48px 56px",
        textAlign: "center", boxShadow: shadows.xl, maxWidth: 440,
      }}>
        <div style={{
        ...
        <h2 style={{ fontSize: 24, fontWeight: 700, color: colors.text.primary, margin: "0 0 8px" }}>
          {isApproved ? "Inspection Approved!" : "Inspection Rejected"}
        </h2>
        <p style={{ fontSize: 16, color: colors.text.secondary, margin: "0 0 6px" }}>
          <strong>{data.irNumber}</strong> — {data.partName}
        </p>
        <p style={{ fontSize: 14, color: colors.text.tertiary, margin: "0 0 20px" }}>
          {isApproved
            ? "Approved for store transfer. Moving to Approved tab..."
            : `Returned to ${data.makerName || "Maker"} for re-inspection. Moving to Rejected tab...`
          }
        </p>
        <div style={{
          width: 120, height: 4, borderRadius: 2, background: colors.neutral[100],
          margin: "0 auto", overflow: "hidden",
        }}>
          <div style={{
            width: "100%", height: "100%", borderRadius: 2,
            background: accentColor,
            animation: "ckProgressShrink 1.8s linear forwards",
          }} />
        </div>
      </div>
    </div>
          ...
          <div style={{ padding: 24 }}>
            {inspections.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 20px" }}>
                {searchQuery ? (
                  <>
                    <Search size={48} style={{ marginBottom: 16, color: colors.neutral[300] }} />
                    <p style={{ fontSize: 17, fontWeight: 600, color: colors.text.secondary }}>No results found</p>
                    <p style={{ fontSize: 14, color: colors.neutral[400] }}>Try adjusting your search query</p>
                  </>
                ) : activeTab === "pending" ? (
                  <>
                    <div style={{ width: 80, height: 80, borderRadius: "50%", background: colors.successBg, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                      <CheckCircle2 size={40} color={colors.success} />
                    </div>
                    <p style={{ fontSize: 18, fontWeight: 700, color: colors.text.primary }}>All caught up!</p>
                    <p style={{ fontSize: 15, color: colors.neutral[500], maxWidth: 360, margin: "8px auto 0" }}>
                      No pending inspections to review. New items will appear here when Makers complete their inspections.
                    </p>
                  </>
                ) : activeTab === "approved" ? (
                  <>
                    <div style={{ width: 80, height: 80, borderRadius: "50%", background: colors.neutral[100], display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                      <CheckCircle2 size={40} color={colors.neutral[300]} />
                    </div>
                    <p style={{ fontSize: 18, fontWeight: 700, color: colors.text.primary }}>No approved inspections yet</p>
                    <p style={{ fontSize: 15, color: colors.neutral[500] }}>Approved items will appear here after you review and approve them.</p>
                  </>
                ) : (
                  <>
                    <div style={{ width: 80, height: 80, borderRadius: "50%", background: colors.neutral[100], display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                    ...
                    <p style={{ fontSize: 18, fontWeight: 700, color: colors.text.primary }}>No rejected inspections</p>
                    <p style={{ fontSize: 15, color: colors.neutral[500] }}>Rejected items will appear here when you reject an inspection and return it to the Maker.</p>
                  </>
                )}
              </div>
            ) : (
      ...
      <div style={{ padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 13, color: colors.neutral[500] }}>
            {ins.status === "pending_review"
              ...
              : ins.status === "approved"
              ? `Approved ${formatDate(ins.checkerDate)}`
              : `Rejected ${formatDate(ins.checkerDate)}`}
          </span>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 20px", fontSize: 14, color: colors.text.secondary }}>
              ...
              <span><strong style={{ color: colors.neutral[500], fontWeight: 500 }}>Vendor:</strong> {ins.vendorName}</span>
            </div>
            <div style={{ display: "flex", gap: 16, marginTop: 10, fontSize: 14, color: colors.text.tertiary }}>
              <span style={{ display: "flex", alignItems: "center", gap: 4 }}><User size={13} /> {ins.makerName}</span>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 14, color: colors.neutral[500], flex: 1, minWidth: 0 }}>
            {}
            {ins.status === "approved" && ins.checkerRemarks ? (
              <div>
                <span style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                  <ShieldCheck size={14} color={colors.success} />
                  <strong style={{ color: colors.success, fontWeight: 600, fontSize: 13 }}>Approved:</strong>
                  <span style={{ fontSize: 12, color: colors.neutral[400] }}>{formatDate(ins.checkerDate)}</span>
                </span>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
            {ins.status === "approved" && (
              <span style={{
                ...
                color: colors.successDark || colors.success, fontSize: 13, fontWeight: 600,
              }}>
                <CheckCircle2 size={14} /> Store Transfer Ready
              </span>
            )}
            {ins.status === "rejected" && (
              <span style={{
            ...
            <button onClick={(e) => { e.stopPropagation(); onView(); }} style={{
              ...
              <Eye size={14} /> {ins.status === "pending_review" ? "Review" : "View Details"}
